...
# --- GPU-dependent file-picker ---
set_based_on_gpu() {
    local gpu_file=$1
    local non_gpu_file=$2
    local selected=""

    printf "${BOLD}Auto-deduction:${RESET}\n"
    if check_gpu; then
...
usage() {
    ...
    echo "  -p   Setup for Podman Quadlet"
    exit 1
}
...
# --- Notify on target ---
printf "{BOLD}Selected target: ${BLUE}$(echo "$TARGET" | tr '[:lower:]' '[:upper:]')${RESET}\n"
...
# Sets up Docker Compose container
setup_docker() {
...
# Sets up Podman Quadlet container
setup_podman() {
